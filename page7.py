...
def home():
    count = 1
    conn=sqlite3.Connection('EmployeeBase.db')
    cur = conn.execute('SELECT * from Person ORDER BY IsJoined')
    Employees = [dict(Id=row[0],
                Name=row[1],
                CorpID=row[2],
                Contact=row[3],
    	...
    	for(x,y,w,h) in faces:
    		...
    		print
                ...
                for (x1,y1,w1,h1) in smiles:
                    ...
                    if str(info[1])== 'Unknown':
                        #print "Entered this yoooo ---------------- - - - - - - - -"
                        ...
                        myCorpId = str(info[2])
                        flag = 1
                        #cv2.cv.PutText(cv2.cv.fromarray(img),'Are you '+str(info[1])+"?",(100,440),new_font,(0,255,0),)
                        #print "my corp id is {}".format(myCorpId)

                if flag == 1:
                    cv2.cv.PutText(cv2.cv.fromarray(img),'Are you '+str(info[1])+"?",(15,40),new_font,(255,255,255),)
                    break
...
def placeTime():
    ...
    query1 = (" UPDATE Person SET ADOJ='%s' WHERE CorpID='%s' " % (dateString,corpID))

    query2 = (" UPDATE Person SET IsJoined='Joined' WHERE CorpID='%s' " % (corpID))

    query3 = (" UPDATE Person SET Time='%s' WHERE CorpID='%s' " % (timeString,corpID))
